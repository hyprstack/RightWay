...
//A ROUTER socket uses the frames from the ZeroMQ Message Transport Protoco; to route each reply message back to the connection that issued the request.
...
const
	...
	if (cluster.isMaster) {
		...
		dealer .on('message', function(){
			//In the book it uses call(null, arguments) - but does not work when called in the terminal.
			//Also in the book when calling the file  zmq-filler-req-loop.js it does not mention the target file - so it should write:
			//node --harmony zmq-filler-req-loop.js target.txt
			let frames = Array.prototype.slice.call(arguments);
			router.send(frames);
		});
